...
class Action
{
	...
	//login admnin and staff user to database
	function login()
	{
		extract($_POST);
	...
	//logout admnin and staff user to database
	function logout()
	{
		session_destroy();
	...
	//save admnin and staff user to database
	function save_user()
	{
		extract($_POST);
		$data = "";
	...
	//update admnin and staff user from database
	...
	function get_parcel_heistory()
	{
		...
		if ($parcel->num_rows <= 0) {
			...
			if ($parcel['status'] == 10) {
				$data[] = array('status' => 'Pending', 'date_created' => date("M d, Y h:i A", strtotime($parcel['date_created'])));
			} else {
